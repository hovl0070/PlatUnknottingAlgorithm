@interact
def NuttyKnotter(p1 = input_box('2',type = int, label='Bridge #: '), p2 = input_box('4 ',type = int, label='Iterations  ')):
    import random
    ...
    #To flip move, choose a random location in plat.
    #To flip bottom (slap the table) between the k-th and (k + 1)-st strands insert into random location in list the following braid word:
    #left=list(range(1,k+1))*k
    #right=list(range(-(n-1),-k+1))*(n-k)
    #flipk=left+right
    # To flop bottom (slap bottom of table) between the k-th and (k + 1)-st strands insert into random location in list the following braid word:
    #left=list(range(-(k-1),0))*k
    #right=list(range(k+1,n-1))*(n-k)
    #flopk=left+right
    #You need to check this because of indexing issues
